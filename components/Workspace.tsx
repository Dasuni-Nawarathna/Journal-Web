...
export default function Workspace() {
    ...
    const handleSavePage = async () => {
        if (!journalText.trim()) {
            setSaveStatus({ type: 'error', text: '🍉 The page is completely empty!' });
            return;
        }

        if (!userId) {
            setSaveStatus({ type: 'error', text: '🔒 Security block: Log in required.' });
            return;
        }
        ...
        try {
            // 🌟 THE MAGIC STEP: Encrypt the text using the userId as a dynamic lock
            // (In production, you can use a custom user PIN here instead!)
                ...
                .insert([
                    {
                        ...
                        content: encryptedContent, // Send the scrambled text to the database!
            ...
            setSaveStatus({ type: 'success', text: '✨ Encrypted & locked away safely inside your secret archive.' });
                    ...
                    <AnimatePresence mode="popLayout">
                        {saveStatus && (
                            <motion.div
                                ...
                                exit={{ opacity: 0, y: -10 }}
                                className={`mb-4 p-3 rounded-xl text-xs font-medium flex items-center gap-2 shadow-sm ${saveStatus.type === 'success' ? 'bg-sage/50 text-espresso' : 'bg-blush/60 text-espresso'
                                    }`}
                            >
                    ...
                    <div className="fixed inset-0 bg-espresso/30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center text-center mt-4 space-y-4">
                                ...
                                {profileMessage && (
                                    <div className={`text-xs px-3 py-1.5 rounded-xl font-medium w-full text-center ${profileMessage.type === 'success' ? 'bg-sage/40 text-espresso' : 'bg-blush/50 text-espresso'
                                        }`}>
                                        {profileMessage.text}
                                    </div>
                                )}
